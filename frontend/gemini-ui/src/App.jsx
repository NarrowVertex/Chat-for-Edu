...
import {
  ...
  MoreVertical, ChevronRight, ChevronDown, Hash, Send, ExternalLink, CornerDownRight, SquarePlus, Trash2
} from 'lucide-react';
import './App.css';
...
function App() {
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      // Existing Project Actions (Default, 1st, 2nd)
      if (!activeChat) {
        console.warn('No active chat selected');
        return;
      }

      const isSubMode = activeIcons.next; // 세부 질문 모드 여부

      // [수정] contextNode가 없을 때(빈 프로젝트)를 위해 generateNodeLabel 호출부 보강
      ...
      // [수정] contextNode가 없는 빈 프로젝트 상태에서도 최상위 노드로 생성되도록 parent_id 명시적 처리
      let finalParentId = "";
      if (contextNode) {
      ...
      if (response.ok) {
        ...
        clearImage();
        setActiveIcons({ next: false, node: false, sparkle: false });
        fetchNodes(activeChat.id, data.id); // 신규 노드 ID를 전달하여 자동 선택
      }
    } catch (err) {
  ...
  return (
    <div className="app-container">
      ...
      <main className="main-content">
        ...
        <section className="center-lobby">
          {view === 'home' ? (
            <>
                  ...
                  <>
                    <div className="node-detail-header">
                      <div className="header-row">
                        ...
                        <div className="action-btns">
                          ...
                          <button className="icon-button"><RotateCcw size={18} /></button>
                        </div>
                      </div>
                    ...
                    <div className="node-content-body">
                      ...
                      <div className="answer-section">
                        ...
                        <div className="text-box ai-answer">{selectedNode.answer_text}</div>
                      </div>
                    </div>
          ...
          <div className="input-area-wrapper">
            ...
            <div className="input-container">
              <input
                ...
                placeholder="Chat for Edu에게 물어보기"
                value={inputText} onChange={(e) => setInputText(e.target.value)}
                onPaste={handlePaste} onKeyDown={(e) => e.key === 'Enter' && handleSendMessage()}
              />
              <div className="input-actions">
                <div className="input-actions-left">
                  <label style={{ cursor: 'pointer' }}>
                    <Paperclip size={20} />
                    <input type="file" style={{ display: 'none' }} onChange={(e) => processImageFile(e.target.files[0])} />
                  </label>
                </div>
                  ...
                  {view === 'project' && (
                    <>
                      ...
                      <button
                        ...
                        title="새로운 시작"
                        onClick={() => toggleSmartIcon('sparkle')}
                      >
                        <Sparkles size={18} />
                      </button>
                    </>
                  )}
                  <button className="icon-button send-btn-main" onClick={handleSendMessage}>
                    <Send size={20} />
                  </button>
                </div>
              </div>
